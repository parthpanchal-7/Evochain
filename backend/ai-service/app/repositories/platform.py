...
class PlatformRepository:
        ...
        for doc in docs:
            ...
            points.append(
                TelemetryPoint(
                    ...
                    block_size=snapshot.last_block_size,
                )
            )
        return points
    ...
    def _build_alerts(
        ...
        if previous and previous.throughput_tps > 20 and snapshot.throughput_tps < previous.throughput_tps * 0.3:
            alerts.append(
                SecurityAlert(
                    id=f"alert-{uuid.uuid4().hex[:10]}",
                    level="critical",
                    kind="throughput-drop",
                    message="Throughput dropped dramatically during high load, investigate possible attack or instability.",
                    node_id=payload.node_id,
                    metrics={
